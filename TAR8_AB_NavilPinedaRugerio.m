...
function generator(individuos, num_individuos, seed)
    ...
    child_mut = zeros(ceil(num_individuos/2)*2,size(b_matriz,2));
    i = 1;
    ...
    while i < (num_iter*2)
        ...
        for j=1:num_padres
            seed = rand(1);
            marcas = ruleta(matriz_probAc, seed, j, marcas);
        end
        fprintf('Padre %d: ', marcas(1)); disp(b_matriz(marcas(1),:));
        fprintf('Padre %d: ', marcas(2)); disp(b_matriz(marcas(2),:));

        % Cruza
        children = cruza(marcas, b_matriz, nb_total);
        %fprintf('\nLos hijos son: \n');
        %fprintf('Cruza\n');
        %disp(children);
        ...
        % Mutacion
        new_children = mutacion(0.5, children, nb_total);
        %fprintf('\nMutacion\n');
        %disp(new_children);

        child_mut(i,:) = new_children(1,:);
        child_mut(i+1,:) = new_children(2,:);
...
function children = cruza(parents, b_matriz, nb_total)
    % Tomar a los individuos mas aptos
    % Por cada padre salen dos hijos
    parent1 = parents(1);
    parent2 = parents(2);

    children = zeros(size(nb_total,1),size(b_matriz,2));

    for k=1:2
        for i=1:size(nb_total, 2)
            child_v1 = zeros(1,nb_total(i));
            child_v2 = zeros(1,nb_total(i));
            cross_point = ceil(nb_total(i)/2);
            child_v1(1,1:cross_point) = b_matriz(parent2, 1:cross_point);
            child_v2(1,1:cross_point) = b_matriz(parent1, 1:cross_point);
            child_v1(1,cross_point+1:nb_total(i)) = b_matriz(parent1, cross_point+1:nb_total(i));
            child_v2(1,cross_point+1:nb_total(i)) = b_matriz(parent2, cross_point+1:nb_total(i));
            children(k,1:nb_total(i)) = child_v1(1,:);
            children(k,nb_total(i)+1:size(b_matriz,2)) = child_v2(1,:);
        end
    end
end

function new_children = mutacion(mut_pt, children, nb_total)
    % Toma un valor random, si el valor es menor al punto de mutacion
    % cambia, sino permanece
    new_children = zeros(1,size(children,2));
    ...
    for k=1:2
        ...
        for i=1:size(nb_total, 2)
            for j=1:nb_total(i)
                ...
                r = round(r, 2);
                if(r < mut_pt)
                    if(children(k,j+cont) == 1)
                        new_children(k,j+cont) = 0;
                    else
                        new_children(k,j+cont) = 1;
                    end
                else
                    new_children(k,j+cont) = children(k,j+cont);
                end
            end
            cont = cont + nb_total(i);
        end
    end
end