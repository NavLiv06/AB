...
for i=1:30
    rand_1_bin(individuos, 10, seed, 0.1, 0.8, 10);
end
...
function rand_1_bin(individuos, num_individuos, seed, mut_factor, cr_factor, num_generaciones)
    ...
    %for i=1:size(nb_total,2)
        %fprintf('Numero de bits qde la variable %d: %d\n', i,nb_total(1,i));
    %end
    ...
    for gen=1:num_generaciones
        ...
        while n <= num_individuos
            ...
            for m=1:num_variables
                ...
                %fprintf('Rand: %d', rand);
                if r > cr_factor
                    cross_child(1,m:jrand-1) = actual(m:jrand-1);
                    break;
                end
            end

            % Cambiar en jrand el ruido
            cross_child(1,jrand) = new_mut(jrand);
        ...
        if may_child < max(hijos(:,num_variables+1))
            may_child = max(hijos(:,num_variables+1));
            may_iter = gen;
        end
    ...
    % maximo_i = find(hijos(:,end)==may_child);
    % maximo = hijos(maximo_i,:);
    % %fprintf('Costo computacional: %d\n', num_individuos * (may_iter-1));
    % %fprintf('Máximo: %d\n', may_child)
    % %fprintf('Ejecucion: %d\n', ejecucion);
    % disp(maximo(1,:));

    may_child = min(hijos(:,end));
    maximo_i = find(hijos(:,end)==may_child);
    maximo = hijos(maximo_i,:);
    %fprintf('Costo computacional: %d\n', num_individuos * (may_iter-1));
    %fprintf('Máximo: %d\n', may_child)
